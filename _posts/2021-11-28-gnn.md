...
输入图 $\mathcal G = (\mathcal V, \mathcal E)$, 和结点特征 $X \in \mathbb R^{d\times |\mathcal V|}$, 生成结点 $u$ 的 embedding $z_u$. 其中 $\mathcal V, \mathcal E$ 分别为全体结点集合和全体边集合, $|\mathcal V|$ 表示集合 $\mathcal V$ 的元素个数. 得到 node embedding 之后, 其他标准的深度学习方法都可以用了.
