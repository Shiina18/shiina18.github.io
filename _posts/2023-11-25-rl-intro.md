...
&= \sum_t \mathbb E_{s_t \sim p_{\theta'}(s_t)} [ \mathbb E_{a_t \sim \pi_{\theta'}(a_t | s_t)} \gamma^t A^{\pi_\theta}(s_t, a_t) ] \\ 
&= \sum_t \mathbb E_{s_t \sim p_{\theta'}(s_t)} \left[ \mathbb E_{a_t \sim \pi_{\theta}(a_t | s_t)} \frac{\pi_{\theta'}(a_t | s_t)}{\pi_{\theta}(a_t | s_t)} \gamma^t A^{\pi_\theta}(s_t, a_t) \right].
\end{align*}
$$
